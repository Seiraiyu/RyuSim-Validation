"""Cocotb testbench for the XuanTie C906 RISC-V core.
...
The top module under test is soc, which wraps the C906 CPU core
(cpu_sub_system_axi -> c906_wrapper -> openC906), AXI interconnect, AXI slave
memory, and peripherals (UART, GPIO, timer, PLIC, CLINT).

Architecture notes:
  - XuanTie C906 is an RV64GC mid-range application core from T-Head (Alibaba)
  - The SoC uses AXI4 bus interfaces for CPU memory access
  - AXI slave memory (axi_slave128) provides instruction and data storage
  - Peripherals are connected via AHB/APB bridges
  - The core has JTAG debug support via tdt_top

Test strategy:
  - test_compile: Validates that RyuSim can parse and compile the full C906
    SoC design (~290 source files). This alone is a major validation milestone.
...
async def test_compile(dut):
    """Verify that the XuanTie C906 SoC design compiles and elaborates successfully.

    This test validates that RyuSim can handle:
    - ~290 Verilog source files totaling ~150K lines of RTL
    - The openC906 CPU core with full RV64GC pipeline
    - AXI4 interconnect and slave memory modules
    - PLIC, CLINT, and peripheral controllers
    - Complex gated clock cells and SRAM models
    - T-Head debug transport module (tdt) hierarchy
    ...
    dut._log.info("XuanTie C906 SoC design compiled and elaborated successfully")
